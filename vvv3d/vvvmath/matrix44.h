...
public:
    ...
    matrix44<T>& loadLookAt(const vector3<T>& eye, const vector3<T>& target, const  vector3<T>& up);
    ...
    {
        return matrix44<T>(*this).transpose();
    }


    static void mul(const matrix44<T>& lhs, const matrix44<T>& rhs, matrix44<T>& out);
    ...
    vector4<T>      operator*  (const vector4<T>&  in) const
    {
        return transform(in);
    }
    ...
    static matrix44<T> createPerspectiveMatrix(T fovy, T aspect, T zNear, T zFar)
    {
        return matrix44<T>().loadPerspective(fovy,aspect,zNear,zFar);
    }

    static matrix44<T> createOrthoMatrix(T left, T right, T bottom, T top, T zNear, T zFar)
    {
        return matrix44<T>().loadOrtho(left,right,bottom,top,zNear,zFar);
    }

    static matrix44<T> createLookAtMatrix(const vector3<T>& eye, const vector3<T>& target, const vector3<T>& up )
    {
    ...
    static matrix44<T> createRotationMatrix(const T& angle, const T& ax, const T& ay, const T& az)
    {
...
{
    stream << "{ { " << m.matrix[0][0] << ", " << m.matrix[0][1] << ", " << m.matrix[0][2] << ", " << m.matrix[0][3] << "}, " << std::endl;
    stream << "  { " << m.matrix[1][0] << ", " << m.matrix[1][1] << ", " << m.matrix[1][2] << ", " << m.matrix[1][3] << "}, " << std::endl;
    stream << "  { " << m.matrix[2][0] << ", " << m.matrix[2][1] << ", " << m.matrix[2][2] << ", " << m.matrix[2][3] << "}, " << std::endl;
    stream << "  { " << m.matrix[3][0] << ", " << m.matrix[3][1] << ", " << m.matrix[3][2] << ", " << m.matrix[3][3] << "} }" << std::endl;
    return stream;
}
