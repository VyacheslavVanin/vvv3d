...
void HorizontalLayout::rearrange()
{
    ...
    for(auto w: children) {
        ...
        case HALIGN::CENTER: offsetY += (contentHeight - w->getHeight()) / 2; break;
...
int HorizontalLayout::getChildrenWidth(const std::vector<Widget*>& children)
{
    return  std::accumulate(children.begin(), children.end(), 0,
                            [](int sum, Widget* w){
                                return sum + w->getSize().x;
                            });
}

HALIGN HorizontalLayout::getAlign() const
{
    return align;
}

void HorizontalLayout::setAlign(HALIGN value)
{
    this->align = value;
}
